...
<button class="lab-placeholder" type="button" onclick="if(confirm('Browser harvest (T1555.003): closes Chrome/Edge, then copies Login Data (saved-password DB), Cookies and Local State (DPAPI-wrapped key) to C:\\LabOut\\Browser. Collection only — files stay in the VM — OK to continue?'))ws.send('LAB:browser')">&#127760; Browser harvest (T1555)</button>
</div>
...
<div class="builder-fields"><select class="kb-send-input" id="quickActionType" onchange="updateQuickActionEditor()"><option value="shortcut">⌨ Keyboard shortcut</option><option value="link">🔗 Open web link</option><option value="paste">📋 Paste text / link</option><option value="run">▶ Win+R command</option><option value="macro">⚡ Macro (multi-step)</option><option value="pwsh">🐚 PowerShell</option></select><input class="kb-send-input" id="quickActionLabel" type="text" maxlength="24" placeholder="Button name" required><input class="kb-send-input" id="quickActionValue" type="text" maxlength="180" placeholder="Shortcut (e.g. Ctrl+Shift+S)" oninput="syncShortcutChips()" required><input class="kb-send-input" id="pwshDelay" type="number" min="500" max="10000" value="1500" placeholder="PS open delay (ms)" style="display:none"><label id="pwshAdminLabel" style="display:none;font-size:.75em;color:#4fd1c5;cursor:pointer;margin-top:4px"><input type="checkbox" id="pwshAdmin" style="accent-color:#4fd1c5"> Run as Admin (UAC)</label><datalist id="builderRunSuggestions"><option value="calc"><option value="notepad"><option value="mspaint"><option value="explorer"></datalist></div>
...
function updateQuickActionEditor() {
    ...
    value.style.display=isMacro?'none':'';
    value.required=!isMacro;
...
function executeStoredAction(action){if(!confirm('Run: '+action.l+'?'))return;if(action.t==='shortcut')executeQuickCombo(action.v,action.l);else if(action.t==='paste'){ws.send('TYPE:'+action.v);footer.textContent='Typed: '+action.l;}else if(action.t==='macro')executeMacro(action.v,action.l);else if(action.t==='pwsh')executePwsh(action.v,action.l);else executeWinR(action.v,action.l);}
